...
- 不把单周专家选择直接升级为普遍规则。
- 不在用户确认前修改 `SKILL.md`、历史模式文档或核心配置；高置信候选仅允许写入 `config/evolution_rules.json` 的 active registry。
- 不用专家稿覆盖系统稿后宣称“自进化完成”。

## 确认后更新优先级

0. `config/evolution_rules.json`：高置信候选自动 active。
1. `docs/HISTORICAL_BUSINESS_REPORT_PATTERNS.md`：稳定写法模式。
2. `config/impact/professional_company_impact_rules.json`：专业公司和业务链路映射。
3. `docs/QUALITY_CHECKLIST.md`：新增质检项。
4. `SKILL.md`：核心流程或硬边界。
